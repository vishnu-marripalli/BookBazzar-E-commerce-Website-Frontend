...
const Login = () => {
  ...
  const handleregister = async (data) => {
    ...
    try {
      setIsLoading(true);
      const response = await ApiCall({
        url: "https://bookbazzar-backend.onrender.com/api/v1/user/register",
        method: "POST",
        data: data,
      });
      setIsLoading(false);
  ...
  const SSOHandler = () => {
    try {
      ...
      // );
      window.open(
        "https://bookbazzar-backend.onrender.com/api/v1/user/google",
        "_self"
      );
    } catch (error) {
          ...
          <div className="shadow-lg border  my-10 border-gray-200 overflow-hidden w-full h-auto  md:w-[25%]">
            <div className="w-full ">
              <div className="flex felx-row text-[#191C1F] font-semibold text-2xl">
                <div
                  className="flex justify-center items-center p-4 w-1/2"
                  onClick={() => {
                    resetSlider();
                    setPage("Signin");
                    signintoggle();
                ...
                >
                  <h1 className="" style={{ userSelect: "none" }}>
                    Sign In
                  </h1>
                </div>
                <div
              ...
              <div
                ref={Sliderref}
                className="border-[2px] w-1/2 border-primary"
              ></div>
            </div>
            <div
              ref={signinref}
              className={
                page === "Signin"
                  ? "opacity-0 translate-x-[100vh]  px-4"
                  : "opacity-0 hidden translate-x-[100vh]  px-4"
              }
            >
              <form onSubmit={handleSubmitForm1(handlelogin)}>
                ...
                />
                {errorsForm1.email && (
                  <p className="text-red-500">{errorsForm1.email.message}</p>
                )}
                <Input
                  label="Password"
                  type="password"
                ...
                {errorsForm1.password && (
                  <p className="text-red-500">{errorsForm1.password.message}</p>
                )}
                ...
                <button
                  type="submit"
                  className="hover:bg-white hover:text-primary  my-4  duration-200 ease-in border-2 border-primary bg-primary w-[100%] px-6 py-2 text-base sm:text-xl font-semibold text-white rounded-sm"
                  onClick={() => { }}
                >
                  Sign In
                </button>
            ...
            >
              <form onSubmit={handleSubmitForm2(handleregister)}>
                ...
                />
                {errorsForm2.name && (
                  <p className="text-red-500">{errorsForm2.name.message}</p>
                )}
                <Input
                  label="Email"
                  // placeholder="Enter your email"
                ...
                />
                {errorsForm2.setemail && (
                  <p className="text-red-500">{errorsForm2.setemail.message}</p>
                )}
                <Input
                  label="Password"
                  type="password"
                  id="setpassword"
                  {...registerForm2("setpassword", {
                    required: "Password is Required",
                    minLength: {
                      value: 6,
                ...
                />
                {errorsForm2.setpassword && (
                  <p className="text-red-500">
                    {errorsForm2.setpassword.message}
                  </p>
                )}
                <Input
                  ...
                  {...registerForm2("setconfirmPassword", {
                    ...
                    validate: (value) =>
                      value === setpassword || "Passwords do not match",
                  })}
                />
                {errorsForm2.setconfirmPassword && (
                  <p className="text-red-500">
                    {errorsForm2.setconfirmPassword.message}
                  </p>
                )}
